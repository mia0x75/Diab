...
INIT_COMPLETED="$CONTAINER_DATA_DIR/.done"
DOLT_CONFIG_DIR="/etc/dolt/dolt.d"
SERVER_CONFIG_DIR="/etc/dolt/server.d"
DOLT_INITDB_DIR="/etc/dolt/init.d"
DOLT_ROOT_PATH="/.dolt"
...
# this function will run files found in /etc/dolt/initdb.d directory BEFORE server is started
...
# if there is config file provided through /etc/dolt/dolt.d,
...
_main() {
  ...
  # dolt config will be set if user provided a single json file in /etc/dolt/dolt.d directory.
  # It will overwrite config_global.json file in $HOME/.dolt
  set_dolt_config_if_defined

  # if there is a single yaml provided in /etc/dolt/server.d directory,
  ...
  if [[ ! -f $INIT_COMPLETED ]]; then
    # run any file provided in /etc/dolt/initdb.d directory before the server starts
    process_init_files /$DOLT_INITDB_DIR/*
